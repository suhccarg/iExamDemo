...
public class ExamButton: UIButton {
    ...
    public init(config: ExamButtonConfig)  throws {
        ...
        _ = log(90, "ExamButton#init(...) :\(config.caption)")
        setup(config: config)
    }//init(...)
    ...
    public func setup(config: ExamButtonConfig) {
        _ = log(90, "ExamButton#setup:\(config.caption)")
        setupColor()
    ...
    public func layout() throws {
        _ = log(90, "ExamButton#layout: \(config!.caption)")
        guard let c = self.config else {
            throw ExamError.runtime("ExamButton#layout: config=nil")
        }
        if c.activeStates.contains(viewState)
            ...
            self.frame = rect
            _ = log(90, "ExamButton#layout() [\(self.tag)]: \(self.frame.size.width) x \(self.frame.size.height) at (\(self.frame.minX), \(self.frame.minY))")
            self.isHidden = false
        } else {
            _ = log(90, "ExamButton#layout() [\(self.tag)]: hidden -> true")
            self.isHidden = true
    ...
    private func getX() throws -> Int {
        ...
        default:
            throw ExamError.runtime("ExamButton#getX: Invalid origin \(String(describing: c.align))")
        }
    }//getX()
    
    private func getY() throws -> Int {
        guard let c = self.config else {
            throw ExamError.runtime("ExamButton#getY: config: nil")
        }
        let safe = getSafeArea(baseView: c.baseView!)
        if let v = config!.upperView {
...
//        var previousView = config.upperView
    ...
    public func select(tag: Int) {
        ...
        do {
            ...
            _ = log(10, "CustomViewController#select(\(tag)):\(e)")
        }
    }//select(sender: UIButton)
...
public class ExamRadioButton: UIButton {
    ...
    init(config: ExamRadioConfig, radioGroup: ExamRadioGroup)  {
        ...
        _ = log(90, "ExamRadioButton#init(...): \(self.tag)")
        setup()
    }//init(...)
    ...
    @objc public func radioButtonClicked(_ sender: UIButton) {
        _ = log(90, "ExamRadioButton#radioButtonClicked(...): \(self.tag)")
    ...
    public func setupButton() {
        ...
        _ = log(90, "ExamRadioButton#setup:\(self.tag)")
...
//        base.scroll.addSubview(self)
    ...
    public func setupLabel() {
        ...
        _ = log(90, "ExamRadioButton#setupLabel: \(label.tag)")
        if debugLevel > 100 {
    ...
    override public func draw(_ rect: CGRect) {
        _ = log(90, "ExamRadioButton#draw:\(self.tag)  \(rect.width) x \(rect.height) at (\(rect.minX), \(rect.minY))")
        super.draw(rect)
        drawFrame()
        drawCheck()
    }//draw(_ rect: CGRect)
...
//        framePath.stroke()
    ...
    public func check(selected: Bool) {
        //setNeedsDisplay()
        _ = log(90, "ExamRadioButton#check(\(selected)): \(self.tag)")
        self.isSelected = selected
        return
    }//check(selected: Bool)
    
    public func layout() throws {
        _ = log(90, "ExamRadioButton#layout: \(self.tag)")
        if self.label != nil {
...
//        let buttonSize = ExamRadioButton.defaultButtonSize
        ...
        _ = log(90, "ExamRadioButton#layoutLabel:\(self.tag) \(self.frame.size.width) x \(self.frame.size.height) at (\(self.frame.minX), \(self.frame.minY)) ")

        label.frame = CGRect.init(x: labelX, y: y, width: labelW, height: h )
        _ = log(90, "ExamRadioButton#layoutLabel:\(label.tag) \(label.frame.size.width) x \(label.frame.size.height) at (\(label.frame.minX), \(label.frame.minY)) ")
//        drawFrame()
//        drawCheck()
    }//layoutLabel()
